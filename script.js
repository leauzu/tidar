(() => {
  ...
  const heroVideoMedia = window.matchMedia('(max-width: 900px)');
  let activeHeroVideoSrc = '';

  const syncHeroVideoSource = () => {
    if (!heroVideo) return;
    const nextSrc = heroVideoMedia.matches
      ? heroVideo.dataset.mobileSrc
      : heroVideo.dataset.desktopSrc;
    if (!nextSrc || nextSrc === activeHeroVideoSrc) return;

    activeHeroVideoSrc = nextSrc;
    heroVideo.pause();
    heroVideo.setAttribute('src', nextSrc);
    heroVideo.dataset.activeSource = heroVideoMedia.matches ? 'mobile' : 'desktop';
    heroVideo.load();

    const playPromise = heroVideo.play();
    if (playPromise && typeof playPromise.catch === 'function') {
      playPromise.catch(() => {
        // Muted autoplay can still be deferred by the browser until the page is visible.
      });
    }
  };

  syncHeroVideoSource();
  if (typeof heroVideoMedia.addEventListener === 'function') {
    heroVideoMedia.addEventListener('change', syncHeroVideoSource);
  } else if (typeof heroVideoMedia.addListener === 'function') {
    heroVideoMedia.addListener(syncHeroVideoSource);
  }
  window.addEventListener('pageshow', syncHeroVideoSource);
